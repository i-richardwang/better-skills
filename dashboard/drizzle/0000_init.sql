...
CREATE TABLE "iterations" (
    ...
    CONSTRAINT "iterations_skill_id_skills_id_fk" FOREIGN KEY ("skill_id") REFERENCES "skills"("id") ON DELETE cascade ON UPDATE no action
);

CREATE UNIQUE INDEX "iterations_skill_iter_key" ON "iterations" USING btree ("skill_id", "iteration_number");
...
CREATE TABLE "runs" (
    ...
    "notes" text[],
    "raw_grading" jsonb,
    CONSTRAINT "runs_iteration_id_iterations_id_fk" FOREIGN KEY ("iteration_id") REFERENCES "iterations"("id") ON DELETE cascade ON UPDATE no action
);

CREATE UNIQUE INDEX "runs_unique_key" ON "runs" USING btree ("iteration_id", "eval_id", "configuration", "run_number");
